...
const FlashSales = () => {
  ...
  const [productForm, setProductForm] = useState({ productId: "", discountPrice: "", quantityLimit: 10 });
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      await userRequest.post(`/flash-sales/${selectedSaleId}/add-product`, productForm);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Button onClick={handleOpenCreate} icon={<FaPlus size={14} />}>Tạo Chiến Dịch Mới</Button>}
      />
            ...
            >
              ...
              <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center border-b px-6 py-4 ${
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleToggleActive(sale)} className="text-xl transition-colors hover:opacity-80 focus:outline-none"
                      ...
                      {sale.isActive ? <FaToggleOn className="text-green-500" /> : <FaToggleOff className="text-gray-400" />}
                    </button>
                    {!sale.isActive && <Badge variant="neutral">ĐÃ TẮT</Badge>}
                  </div>
                ...
                <div className="mt-4 sm:mt-0 flex gap-2">
                  <Button size="sm" variant="secondary" onClick={() => handleOpenEdit(sale)} icon={<FaEdit size={12} />}>Sửa</Button>
                  <IconButton variant="delete" icon={<FaTrash size={12} />} onClick={() => handleDeleteSale(sale._id)} />
                </div>
              </div>
                            ...
                            <tr key={idx} className="hover:bg-gray-50/60 transition-colors">
                              <td className="px-5 py-3 flex items-center gap-3">
                                <img src={item.product?.img || "https://placehold.co/40x56"}
                                  className="h-14 w-10 rounded border border-gray-200 object-cover shadow-sm bg-gray-100" alt="" />
                                <div>
                                  <div className="font-semibold text-gray-800 line-clamp-1 max-w-[200px]" title={item.product?.title}>
                                    {item.product?.title}
                                  </div>
                                  <div className="text-xs text-gray-400 mt-0.5">ID: {item.product?._id.slice(-6)}</div>
                                </div>
                              </td>
                              <td className="px-5 py-3 text-gray-400 line-through">{item.product?.originalPrice?.toLocaleString()} đ</td>
                              <td className="px-5 py-3 font-bold text-red-600 text-base">{item.discountPrice?.toLocaleString()} đ</td>
                              <td className="px-5 py-3 font-medium text-gray-700">{item.quantityLimit}</td>
                              <td className="px-5 py-3 w-40">
                                ...
                                <div className="text-right text-[11px] font-medium text-gray-500">Đã bán {item.soldCount}</div>
                              </td>
                              <td className="px-5 py-3 text-center">
                                <IconButton variant="default" icon={<FaTimes size={13} />}
                                  onClick={() => handleRemoveProduct(sale._id, item.product?._id)}
                                  className="text-gray-400 hover:bg-red-50 hover:text-red-500 hover:border-red-100" />
                              </td>
                            </tr>
                          ))}
                        </tbody>
      ...
      <Modal isOpen={showSaleModal} onClose={() => setShowSaleModal(false)}
        ...
        <form onSubmit={handleSaveSale} className="p-6 space-y-4">
          <InputField label="Tên chiến dịch" required placeholder="VD: Sale Tết 2025"
            value={saleForm.name} onChange={(e) => setSaleForm({ ...saleForm, name: e.target.value })} />
          <div className="grid grid-cols-2 gap-4">
            <InputField label="Bắt đầu" required type="datetime-local"
              value={saleForm.startTime} onChange={(e) => setSaleForm({ ...saleForm, startTime: e.target.value })} />
            <InputField label="Kết thúc" required type="datetime-local"
              value={saleForm.endTime} onChange={(e) => setSaleForm({ ...saleForm, endTime: e.target.value })} />
          </div>
          {editingSaleId && (
          ...
          <div className="mt-6 flex justify-end gap-3 border-t border-gray-100 pt-4">
            <Button variant="secondary" onClick={() => setShowSaleModal(false)}>Hủy</Button>
            <Button type="submit">{editingSaleId ? "LƯU THAY ĐỔI" : "TẠO CHIẾN DỊCH"}</Button>
          </div>
        </form>
      </Modal>

      {/* Modal Thêm Sản Phẩm */}
      <Modal isOpen={showProductModal} onClose={() => setShowProductModal(false)} title="Thêm Sách Vào Sale" size="lg">
        <form onSubmit={handleAddProduct} className="p-6 space-y-4">
          <InputField label="Chọn sách" as="select" required
            value={productForm.productId} onChange={(e) => setProductForm({ ...productForm, productId: e.target.value })}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <InputField label="Giá Sale (VND)" required type="number" placeholder="50000"
              value={productForm.discountPrice} onChange={(e) => setProductForm({ ...productForm, discountPrice: e.target.value })} />
            <InputField label="Số lượng bán" required type="number"
              value={productForm.quantityLimit} onChange={(e) => setProductForm({ ...productForm, quantityLimit: e.target.value })} />
          </div>
          <div className="mt-6 flex justify-end gap-3 border-t border-gray-100 pt-4">
