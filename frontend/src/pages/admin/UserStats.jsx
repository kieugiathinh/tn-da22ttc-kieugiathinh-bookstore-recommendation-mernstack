      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-extrabold text-gray-900">Khách hàng mới theo tháng</h3>
          </div>
          <p className="text-xs text-gray-400 font-semibold mb-4 uppercase tracking-wider">Năm {new Date().getFullYear()}</p>
          <div className="h-[280px]">
            {data?.newUsersByMonth && (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.newUsersByMonth} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="count" name="Khách mới" stroke="#f97316" strokeWidth={3} fillOpacity={1} fill="url(#colorUsers)" activeDot={{r: 6, strokeWidth: 0, fill: '#f97316'}} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
                ...
                <tr key={i} className="hover:bg-orange-50/30 transition-colors group">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-orange-500 to-amber-500 text-sm font-black text-white shadow-md group-hover:scale-110 transition-transform">
                        {c.name?.charAt(0)?.toUpperCase() || "K"}
                      </div>
                      <div>
                        <p className="font-bold text-gray-800 group-hover:text-orange-600 transition-colors">{c.name || "Khách vãng lai"}</p>
                        <p className="text-[11px] text-gray-400">{c.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-right font-black text-emerald-600">{c.totalSpent?.toLocaleString("vi-VN")} ₫</td>
                  <td className="px-4 py-3.5 text-right font-bold text-gray-600">{c.orderCount}</td>
                </tr>
              ))}
